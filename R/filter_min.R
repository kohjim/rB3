filter_min <-  function(DF_in, taskInf) {
  ######## import functions ########
  ...
  ######## makeVarsList ########
  argNames <- c("StartDate","EndDate","VarNames","filename","minVal")
  ...
  for (i in 1:length(colLocsNums)){
    ...
    if (isUseCndFile){
      ...
      rowsToChange <- which(DF_in[,colLocsNums[i]] < as.numeric(as.character(minVal)))
