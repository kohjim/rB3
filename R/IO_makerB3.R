#' Convert a text file of time-series observations into an rB3 object (list of dataframes)
#'
#' Imports a file to rB3 object
#'
#' Creates a list of datatables, including: \cr
#'      1. 'src'  ; the unmodified data \cr
#'      2. 'qc'   ; a copy of the data, which will be modified by rB3\cr
#'      3. 'log'  ; a 'log' file with similar structure to srcDF and qcDF, to store QC operations log\cr
#'      4. 'key'  ; control key with logIDs and explanatory columns
#'      5. 'ctrls'; controls, which can be extracted from extra header rows in the raw csv files ('ctrls')\cr
#'      6. 'meta'  ; site metadata, create from additional input args ('metaD')\cr
#'
...
#' @keywords fileIO
#' @examples IO_makerB3("Rotorua.csv", siteName = "Lake_Rotorua", lat = -38, long = 176, country = "NZ")


IO_makerB3 <- function(filePath, siteName, lat, lon, country) {

  # import as data.table (fast)
  ...
  src <- srcData[hRow:nrow(srcData),]

  # make the headers into colnames and drop from dataset
  colnames(src) <- as.character(unlist(src[1,]))
  src <- src[-1,]

  # define ISO date format
  src$DateTime <-  as.POSIXct(src$DateTime,
